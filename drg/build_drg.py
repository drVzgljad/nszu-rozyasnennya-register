...
#   altw_days ЗАМІНЮЄ ваговий коефіцієнт формулою a + b × дні (коротке лікування);
...
CASE_FACTORS = [
    ...
    {"id": "short_stay", "sub": "9-10", "kind": "altw_days", "on": False,
     "max_days": 30, "table": "shortStay",
     "label": "Коротке лікування: за оперативне втручання і за кожен день",
     "packages": ["3", "4"],
     "note": "Тривалість менша за нижнє референтне значення. У постанові "
             "величин немає — їх визначають алгоритми і правила НСЗУ. ЕСОЗ "
             "рахує обидва підпункти одним коефіцієнтом, який ЗАМІНЮЄ ваговий: "
             "коефіцієнт за втручання + коефіцієнт за добу × дні. Значення "
             "відновлено з фактичних оплат, а не взято з нормативного акта, "
             "і відомі лише для тих ДСГ, які трапилися у вигрузці. У самій "
             "вигрузці цей коефіцієнт має ще два режими — дорівнює вазі "
             "(зменшення немає) або вазі × 0,6; за якою ознакою ЕСОЗ обирає "
             "режим, з даних не видно, тому розділ рахує лише лінійний."},
    ...
    {"id": "icu_day", "sub": "16", "kind": "rateday", "value": 0.1, "on": False,
     ...
     "note": "0,1 за добу, починаючи з другої, загалом не більше семи діб. "
             "Постанова каже «додається до коригувального коефіцієнта», але "
             "фактично ЕСОЗ платить це поверх випадку і БЕЗ частки "
             "застосування 0,55 — перевірено на 472 випадках вигрузки, де "
             "інакше сума не сходиться. Не застосовується до ДСГ з інвазивною "
             "вентиляцією. Для надкластерних і кластерних закладів та закладів "
             "державної форми власності, у ВІТ яких 20 % і більше пацієнтів "
             "були на ШВЛ."},
...
# ── Коефіцієнти короткого лікування (підпункти 9 і 10) ─────────────────────
# Єдине місце в розділі, де числа взято НЕ з нормативного акта.
#
# У постанові величин немає — там сказано лише, що їх визначають алгоритми і
# правила НСЗУ. Але вигрузка ЕСОЗ «Дані_хірургія_2026_for_scores» (60 740
# випадків пакета 3 за 2026 рік) віддає в колонці «Коефіцієнт» фактично
# застосований less_min_bound_base по кожному випадку. За тривалістю лікування
# він лягає на пряму: less_min_bound = a + b × дні, точно, без залишку.
# Звідси a — коефіцієнт за оперативне втручання (підпункт 9),
#        b — базовий коефіцієнт за добу (підпункт 10).
#
# Але прямою описується не кожен короткий випадок. У 2 916 випадках вигрузки
# less_min_bound набуває значень трьох різних режимів:
#   1 964  дорівнює ваговому коефіцієнту — зменшення немає взагалі;
#     934  лінійний a + b × дні — власне коефіцієнти підпунктів 9 і 10;
#      18  дорівнює вазі × 0,6 — амбулаторно-асоційований коефіцієнт
#           підпункту 13 вкладено всередину, окремим множником його немає.
# За якою ознакою ЕСОЗ обирає режим, з вигрузки не видно: нижніх референтних
# значень тривалості в ній немає. Тому таблиця описує лише третій режим,
# і в цьому розділ чесно зізнається.
#
# Групи, де при одній добі коефіцієнт дорівнює ваговому (D04, D14, H08, J10,
# J12), у таблицю не внесені: одна точка прямої не задає, і зменшення там
# фактично не відбувається.
#
# Джерело перевіряється відтворенням: з цими значеннями повна формула
# збігається з фактичною оплатою в 60 595 випадках із 60 595 до копійки.
SHORT_STAY = {
    "D03": {"a": 0.538889, "b": 0.628704, "n": 2},
    "F11": {"a": 0.221563, "b": 0.309406, "n": 4},
    "F13": {"a": 0.170358, "b": 0.357128, "n": 3},
    "H01": {"a": 1.232521, "b": 0.626896, "n": 3},
    "H07": {"a": 0.722889, "b": 0.489528, "n": 3},
    "J01": {"a": 1.956188, "b": 0.891203, "n": 3},
    "J06": {"a": 0.591592, "b": 0.266602, "n": 3},
}

SHORT_STAY_SOURCE = {
    "origin": "Відновлено з фактичних оплат ЕСОЗ, не з нормативного акта.",
    "dataset": "Дані_хірургія_2026_for_scores, 60 740 випадків пакета 3 за 2026 рік",
    "method": "less_min_bound_base лінійний за тривалістю: a + b × дні; "
              "a і b відновлено за двома і більше точками прямої",
    "verified": "повна формула відтворює 60 595 фактичних оплат із 60 595 до копійки",
    "columns": {"a": "підпункт 9 — за оперативне втручання",
                "b": "підпункт 10 — за кожен день"},
    "regimes": [
        {"rule": "= ваговий коефіцієнт", "cases": 1964,
         "meaning": "зменшення не застосовується"},
        {"rule": "a + b × дні", "cases": 934,
         "meaning": "власне коефіцієнти підпунктів 9 і 10 — цей режим і рахує розділ"},
        {"rule": "= ваговий коефіцієнт × 0,6", "cases": 18,
         "meaning": "амбулаторно-асоційований коефіцієнт підпункту 13 вкладено "
                    "всередину, окремим множником його немає"},
    ],
    "note": "Значення відомі лише для ДСГ, які трапилися у вигрузці. "
            "Для решти груп розділ, як і раніше, не рахує цей коефіцієнт. "
            "Ознаки вибору режиму у вигрузці немає — нижніх референтних "
            "значень тривалості вона не містить.",
}
...
LEADING_DRG = re.compile(r"^(\d{3}|[A-Z]\d{2}[A-Z]?)\s")


def service_drgs(service, by_code):
    """Перелік ДСГ послуги, з підбором коду з назви для рядків-кошиків.

    Рядок «801 Загальні втручання, не пов'язані з основним діагнозом» несе код
    ДСГ у самій назві: колонки коду й коефіцієнтів у нього порожні, бо група не
    має ані діагнозів, ані інтервенцій - вона спрацьовує залишком. Парсер
    Таблиці через це лишає `code` і `drg` порожніми, і група 801 випадала з
    прив'язки: без послуг, без пакетів, ще й у списку дір як «група без ваги».
    Ваговий коефіцієнт у неї є (додаток 1), тому це була хибна діра.

    Підбираємо код з початку назви лише тоді, коли явних даних немає взагалі, і
    лише якщо такий код справді існує серед груп. На Таблиці 2026 року правило
    зачіпає рівно один рядок; решта послуг без коду - це пакети, які взагалі не
    оплачуються за додатками (інсульт, інфаркт, пологи, амбулаторка).
    """
    if service.get("drg"):
        return service["drg"]
    match = LEADING_DRG.match(service.get("name") or "")
    if not match:
        return []
    code = match.group(1).translate(CYR2LAT)
    if code not in by_code:
        return []
    return [{"c": code, "t": "", "k": [], "ka": "", "kl": False}]
...
def attach_services(groups, services):
    ...
    for service in services:
        ...
        for drg in service_drgs(service, by_code):
...
def main():
    ...
    # Дивимося на той самий перелік ДСГ, за яким робилася прив'язка: інакше
    # рядок-кошик 801, чий код узято з назви, лишався б у списку дір.
    linked_codes = {g["c"] for g in groups.values()}
    no_drg = [{"i": s["i"], "n": s["name"][:160], "p": s["pkgs"],
               "why": classify_gap(s["pkgs"])}
              for s in services if not service_drgs(s, linked_codes)]
    ...
    payload = {
        ...
        "shortStaySource": SHORT_STAY_SOURCE,
        "refs": refs,
