...
-- =========================================================================
-- МІГРАЦІЯ: НОВИНА ПРО ІНТЕГРАЦІЮ СТАНДАРТІВ ДЕЦ МОЗ ТА ПОДВІЙНЕ ОНОВЛЕННЯ
-- =========================================================================
INSERT INTO public.news (title, summary, content, tags, importance)
VALUES (
  'Зв''язування стандартів ДЕЦ МОЗ із пакетами ПМГ-2026 та подвійне оновлення',
  'Реалізовано перехресні зв''язки між клінічними протоколами та вимогами закупівлі послуг. Впроваджено подвійний механізм оновлення реєстру: локальний CMD-скрипт та хмарна веб-панель.',
  '## Поєднання клінічних стандартів та вимог закупівлі послуг

Ми раді представити масштабне оновлення нашого робочого простору, спрямоване на синхронізацію нормативних вимог закупівлі медичних послуг (ПМГ 2026) та клінічних протоколів лікування (ДЕЦ МОЗ України). 

Це нововведення дозволить експертам у кілька кліків переходити від медико-технологічних документів до вимог договорів і навпаки.

### Що змінилося:

1. **Інтеграція ПМГ $\leftrightarrow$ ДЕЦ:**
   - **У картках пакетів:** На сторінці кожного пакета ПМГ-2026 з''явився новий блок «Клінічні стандарти та настанови ДЕЦ МОЗ». У ньому автоматично виводяться чинні накази та протоколи ДЕЦ, що відповідають обраному пакету (інсульт, інфаркт, онкологія тощо).
   - **У картках ДЕЦ:** У сайдбарі детального перегляду кожного клінічного стандарту додано новий розділ «Пов''язані пакети ПМГ 2026» із швидким переходом до відповідних вимог договору.

2. **Гібридне завантаження даних (Надійність):**
   - Обидва модулі («Пакети 2026» та «ДЕЦ МОЗ») тепер автоматично намагаються завантажити актуальні дані в реальному часі з хмари Supabase. 
   - У разі відсутності інтернету або очищення бази додаток безперешкодно переходить на локальний файл `dec_documents.json`, забезпечуючи 100% стабільність роботи.

3. **Два варіанти оновлення реєстру:**
   - **Варіант А (Локальний скрипт):** У кореневій папці з''явився файл `Оновити_базу_ДЕЦ.cmd`. Запуск скрипту в один клік розпарсить новий Excel, оновить локальні JSON-файли, синхронізує їх та опублікує на GitHub.
   - **Варіант Б (Панель оновлення у браузері):** Для користувачів із роллю Керівник (`manager` і вище) у правому верхньому кутку сторінки ДЕЦ з''явилася кнопка `⚙️ Оновити базу`. Вона перенаправляє на сторінку `dec/admin.html`, де можна просто перетягнути новий Excel файл та завантажити його у Supabase пачками по 100 рядків із зручним індикатором прогресу.

Новий функціонал повністю сумісний із темною та світлою темами порталу та адаптований для мобільних пристроїв. Бажаємо приємного та ефективного користування!',
  ARRAY['Оголошення', 'ДЕЦ МОЗ', 'Оновлення'],
  'important'
);
