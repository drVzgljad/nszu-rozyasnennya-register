...
(() => {
  ...
  /* Наслідки і сам стан в одному епізоді. МКХ-10 тримає для наслідків окремі
     рубрики, і кожна означає, що гострого стану вже НЕМАЄ: I69 «наслідки
     цереброваскулярних хвороб», T90-T98 «наслідки травм», G09, B90-B94 тощо.
     Коли поруч стоїть і наслідок, і сам гострий стан, це або помилка запису,
     або наслідки ІНШОГО, попереднього епізоду — і тоді це має бути видно з
     документації, а не вгадуватись. Різниця між цими двома випадками для нас
     видима: підкод I69 прямо вказує, наслідком чого він є. */
  const SEQ_RULES = [
    { seq: 'B90', acute: [['A15', 'A19']], what: 'туберкульозу' },
    { seq: 'B91', acute: [['A80', 'A80']], what: 'поліомієліту' },
    { seq: 'B92', acute: [['A30', 'A30']], what: 'лепри' },
    { seq: 'B94', acute: [['A71', 'A71'], ['A83', 'A86'], ['B15', 'B19']],
      what: 'інфекційних і паразитарних хвороб' },
    { seq: 'E64', acute: [['E40', 'E46'], ['E50', 'E63']], what: 'недостатності харчування' },
    { seq: 'E68', acute: [['E65', 'E67']], what: 'надлишковості харчування' },
    { seq: 'G09', acute: [['G00', 'G08']], what: 'запальних хвороб центральної нервової системи' },
    { seq: 'I69', acute: [['I60', 'I67']], what: 'цереброваскулярних хвороб',
      exact: { 0: 'I60', 1: 'I61', 2: 'I62', 3: 'I63', 4: 'I64' } },
    { seq: 'O97', acute: [['O00', 'O95']], what: 'акушерських причин' },
    { seq: 'T90', acute: [['S00', 'S09']], what: 'травми голови' },
    { seq: 'T91', acute: [['S10', 'S39']], what: 'травми шиї та тулуба' },
    { seq: 'T92', acute: [['S40', 'S69']], what: 'травми верхньої кінцівки' },
    { seq: 'T93', acute: [['S70', 'S99']], what: 'травми нижньої кінцівки' },
    { seq: 'T94', acute: [['S00', 'T14']], what: 'множинних травм' },
    { seq: 'T95', acute: [['T20', 'T32']], what: 'опіків' },
    { seq: 'T96', acute: [['T36', 'T50']], what: 'отруєння лікарськими засобами' },
    { seq: 'T97', acute: [['T51', 'T65']], what: 'токсичної дії речовин' },
    { seq: 'Y85', acute: [['V01', 'Y84']], what: 'зовнішніх причин' },
  ];

  /* Перевірка наслідків не має залежати від того, чи бере код участь у
     групуванні: струс мозку S06.0 у Таблиці співставлення відсутній, але
     поруч із T90.5 «наслідки внутрішньочерепної травми» це та сама помилка,
     що й I63 з I69. Тому беремо все, що має форму коду МКХ-10, разом із
     нерозпізнаним. */
  const ICD_RE = /^[A-Z]\d{2}(\.\d+)?$/;
  const dxLike = (q) => [...q.dx, ...q.unknown.filter((c) => ICD_RE.test(c))];

  function sequelaeNote(all) {
    const rub = (c) => c.slice(0, 3);
    const sub = (c) => (c.includes('.') ? c.split('.')[1][0] : '');
    const out = [];
    for (const rule of SEQ_RULES) {
      const seqCodes = all.filter((c) => rub(c) === rule.seq);
      if (!seqCodes.length) continue;
      const acute = all.filter((c) => rule.acute.some(([a, b]) => rub(c) >= a && rub(c) <= b));
      if (!acute.length) continue;
      for (const s of seqCodes) {
        const want = rule.exact && rule.exact[sub(s)];
        const same = want ? acute.filter((c) => rub(c) === want) : [];
        const other = acute.filter((c) => !same.includes(c));
        if (same.length) {
          out.push(`<div class="kd-flag kd-flag-no">
            <b>${esc(s)} і ${same.map(esc).join(', ')} — той самий стан двічі.</b>
            ${esc(s)} за МКХ-10 позначає <b>наслідки</b> ${esc(rule.what)}, тобто
            епізоду, якого вже немає, а ${same.map(esc).join(', ')} — сам цей
            стан у гострому періоді. В одному випадку вони не поєднуються:
            або лікували гострий стан, або його наслідки.
            <span class="kd-src">МКХ-10, том 2, п. 4.5.2, кодування наслідків</span></div>`);
        } else if (other.length) {
          out.push(`<div class="kd-flag kd-flag-warn">
            <b>${esc(s)} — наслідки ${esc(rule.what)} — поруч із гострим
            ${other.map(esc).join(', ')}.</b> Це можливо, якщо йдеться про
            наслідки <i>попереднього</i> епізоду, а не поточного. Тоді так і має
            бути записано в документації: інакше перевірка прочитає це як
            подвійне кодування того самого стану.</div>`);
        }
      }
    }
    return out.join('');
