...
CREATE TABLE IF NOT EXISTS public.package_assignments (
  ...
  -- № пакета за постановою 1808 (текстом; № 86 існує двічі — основний і пілотний)
  num TEXT NOT NULL,
  title TEXT NOT NULL,
...
INSERT INTO public.package_assignments (num, title, resp, dup, pilot, sort_order) VALUES
  ...
  ('86', 'Довготривалий медичний догляд окремим категоріям осіб, які захищали незалежність, суверенітет та територіальну цілісність України', 'koval', 'volosh', true, 520)
