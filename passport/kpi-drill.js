...
(function () {
  ...
  function computeCore(pkgNum) {
    ...
    return {
      ...
      tiny: paid.length <= 6,
      // Поріг мережі опущено з 12 до 7 (07.09.2026): між 7 і 11 закладами
      // ознака не могла спрацювати ніяк, і пакет із десятьма однаковими
      // договорами мовчки діставав вердикт про концентрацію — тобто про
      // рішення, якого там немає. Решта умов не змінена: потрібні і рівне
      // ядро (>= 55 %), і мізерна кількість різних сум, тож на 7 договорах
      // це означає «не більше трьох різних сум». Нижче 7 працює `tiny`.
      flat: paid.length >= 7 && uniq <= Math.max(3, paid.length * 0.08)
  ...
  function funnelSvg(s) {
    ...
    <svg class="dr-funnel${compact ? " compact" : ""}" viewBox="0 0 ${W} ${H}" role="img"
      ...
      ${lbl(0, H - (compact ? 14 : 4), `СУМА ДОГОВОРІВ · ${escapeHtml(formatMoneyShort(s.total))}`, "dr-axtitle", "start")}
    </svg>`;
  }
  ...
  function curveSvg(s) {
    ...
    <svg class="dr-curve${compact ? " compact" : ""}" viewBox="0 0 ${W} ${H}" role="img"
      ...
      <text x="${dl.x.toFixed(1)}" y="${dl.y.toFixed(1)}" class="dr-equal-lbl" text-anchor="middle"
            transform="rotate(-31 ${dl.x.toFixed(1)} ${dl.y.toFixed(1)})">якби суми ділилися порівну</text>
      ...
      <text x="0" y="0" class="dr-axname" transform="translate(${compact ? 22 : 15} ${T + ph / 2}) rotate(-90)" text-anchor="middle">накопичена частка суми договорів</text>
    </svg>`;
  }
  ...
  function verdictOf(s) {
    ...
    if (s.corePct < 25) return {
      icon: "🎯", key: "tight",
      title: "Суми договорів зібрані у вузькому колі закладів",
      desc: "Пакет тримається на кількох центрах: рішення щодо них рухає суму договорів пакета, рішення щодо решти мережі — майже ні.",
    };
    if (s.corePct < 40) return {
      icon: "🔻", key: "dense",
      title: "Суми договорів зібрані у меншості мережі",
      desc: "На чверть-третину закладів припадає 4/5 суми договорів. Типова форма для пакета, де поруч працюють великі й малі надавачі.",
    };
    if (s.corePct < 55) return {
      icon: "📊", key: "even",
      title: "Суми договорів розподілені досить рівномірно",
      desc: "Ядро близьке до половини мережі: явних центрів, на які можна впливати точково, у пакеті немає.",
    };
    return {
      icon: "🫧", key: "spread",
      title: "Суми договорів розмиті по всій мережі",
  ...
  function quantTile(s, q, bigLabel) {
    ...
    const sub = s.n >= 20
      ? `${uaNum(q.k)} ${najb(q.k)} ${zozUk(q.k)} — стільки суми договорів`
  ...
  function meaningHtml(s, v) {
    ...
    if (v.key === "tiny") {
      ...
      items.push(["🎚️", "Бюджет пакета вирішує меншість мережі",
        `Зміна тарифу, коефіцієнта чи умов закупівлі, що зачіпає ${uaNum(s.core)} ${zozUk(s.core)} ядра, рухає ${pctUk(s.coreMoneyPct)} суми договорів пакета. Те саме рішення щодо решти ${uaNum(nT)} ${zozUk(nT)} зачепить лише ${pctUk(tailShare)}.`]);
      ...
      items.push(["🔍", "Нагляд коштує дорого",
        `Вибіркова перевірка найбільших покриє мало: топ-10 % мережі — це лише ${pctUk(s.q10.pct)} суми договорів пакета.`]);
    }
  ...
  /* Дати джерел приходять у двох форматах: вивантажка договорів — «24.08.2026»,
     конвеєри обсягів і демографії — ISO. Зводимо до одного вигляду. */
  function dmy(v) {
    const s = String(v || "");
  ...
  function daysBetween(a, b) {
    const p = (v) => {
      const s = dmy(v).match(/^(\d{2})\.(\d{2})\.(\d{4})$/);
      return s ? new Date(+s[3], +s[2] - 1, +s[1]) : null;
    };
    const x = p(a), y = p(b);
  ...
  const DRILLS = {
    ...
    /* «Джерела та межі даних» — з аудиту 29.08.2026. Вкладка стоїть на пʼяти
       джерелах різного віку, і жодне з них не про результат лікування; окремо
       треба було зняти хибне прочитання глобальних 34,7 % зшивки сум — див.
       картку «Суми»: перевірка 30.08 показала, що це не побиті дані. */
    sources: {
      icon: "🔎",
      title: "Джерела та межі даних",
      build(pkg) {
        const cd = passportState.contractsData || {};
        if (!cd.contracts) return null;

        // Договори саме цього пакета: у скількох із них є сума
        let net = 0, paid = 0, total = 0, fop = 0, fopPaid = 0;
        cd.contracts.forEach((c) => {
          const p = c.packages.find((x) => x.package_num === pkg.number);
          if (!p) return;
          net++;
          const isFop = !/^\d+$/.test(String(c.edrpou || ""));
          if (isFop) fop++;
          if (p.sum > 0) { paid++; total += p.sum; if (isFop) fopPaid++; }
        });
        const cov = net ? (paid / net) * 100 : 0;

        const vm = window.Volumes && window.Volumes.meta ? window.Volumes.meta() : null;
        const dem = window.Volumes && window.Volumes.demo ? window.Volumes.demo() : null;
        const lag = daysBetween(cd.sums_date, cd.source_date);

        const src = (icon, name, date, gives, blind) => `
          <tr>
            <td class="drs-n"><span>${icon}</span>${escapeHtml(name)}</td>
            <td class="drs-d">${escapeHtml(date || "—")}</td>
            <td>${escapeHtml(gives)}</td>
            <td class="drs-b">${escapeHtml(blind)}</td>
          </tr>`;

        return {
          state: null,
          html: `
        <section class="dr-lede">
          <div class="dr-lede-main">
            <p class="dr-lede-q">На чому стоять числа цієї вкладки</p>
            <p class="dr-lede-a">${escapeHtml(
              "П'ять джерел різного віку" +
              (lag ? `; найстаріше — суми договорів, вони на ${uaNum(lag)} ${plural(lag, "день", "дні", "днів")} старші за склад мережі` : "") + ".")}</p>
            <p class="dr-lede-b">Жодне з них не містить <b>результату лікування</b> і <b>унікальних пацієнтів</b>.
              Тому вкладка чесно відповідає на питання «скільки законтрактовано і скільки зроблено» —
              і не може відповісти на питання «чи стало людям краще». Це межа даних, а не недогляд розрахунку.</p>
          </div>
        </section>

        <section class="dr-card">
          <h3>Звідки що береться</h3>
          <div class="drs-wrap">
            <table class="drs">
              <thead><tr><th>Джерело</th><th>Станом на</th><th>Що з нього на вкладці</th><th>Чого в ньому немає</th></tr></thead>
              <tbody>
                ${src("🏥", "Реєстр договорів", dmy(cd.source_date),
                  "склад мережі, області, форма власності, спроможна мережа, перетини пакетів",
                  "строків дії окремих пакетів у договорі; ЗОЗ тут — юрособа, а не місце надання")}
                ${src("💰", "Суми договорів", dmy(cd.sums_date || cd.source_date),
                  "бюджет пакета, медіанний договір, ядро 80 %, вісь грошей",
                  "фактично сплачених грошей: це законтрактована сума, тобто план, а не факт")}
                ${vm ? src("📈", "Фактичні обсяги ЕСОЗ", dmy(vm.generated),
                  "блок «Фактично надано», інтенсивність, режим карти «обсяг послуг»",
                  "унікальних пацієнтів (рахуються послуги, не люди) і місця проживання пацієнта") : ""}
                ${dem ? src("👥", "Декларації ПМД", dmy(dem.declarations_updated),
                  "знаменник режиму «на населення» та інтенсивності",
                  "чисельності населення: офіційної немає, декларації покривають " + pctUk(dem.coverage_ukr_pct || 0) + " бази 2022 року") : ""}
                ${src("📄", "Специфікації пакетів", dmy(passportState.packagesGenerated),
                  "назва й умови пакета, вимоги, анатомія (поріг входу, клінічна ширина)",
                  "змін, ухвалених після цієї дати: файл перезбирається окремо")}
              </tbody>
            </table>
          </div>
          <p class="dr-hint">Дати різні, тому числа з різних джерел не можна ділити одне на одне як однорідні:
            заклад, що уклав договір після ${escapeHtml(dmy(cd.sums_date || "—"))}, у мережі вже є, а суми в нього ще немає.</p>
        </section>

        <section class="dr-card">
          <h3>Суми: за скільки договорів вони взагалі є</h3>
          <section class="dr-stats">
            ${statCard("Договорів пакета", uaNum(net), "у вивантажці складу мережі")}
            ${statCard("З них із сумою", uaNum(paid), cov >= 99.95 ? "суми є за всіма" : `решта ${uaNum(net - paid)} — без суми`)}
            ${statCard("Покриття сум", pctUk(cov), "саме цього пакета, а не ПМГ загалом")}
            ${statCard("Бюджет пакета", formatMoneyShort(total), "сума за договорами, які її мають")}
          </section>
          <p class="dr-hint"><b>Чому у вивантажці загалом лише ${escapeHtml(pctUk(cd.sums_matched_pct || 0))}.</b>
            Це не побита зшивка: серед ${escapeHtml(uaNum(cd.count || 0))} рядків договорів більшість без сум — це
            реімбурсація «Доступні ліки» та пілоти, які оплачуються окремим набором (аптеки), і нулі там стоять
            за задумом. Усередині пакетів постанови № 1808 покриття 85–100 %, а в цього пакета —
            ${escapeHtml(pctUk(cov))}. Перевірено 30.08.2026 звіркою з відкритим набором фактичних оплат:
            додавання оплат підняло б загальне покриття з 34,7 % лише до 34,8 %.</p>
        </section>

        ${fop ? `
        <section class="dr-card">
          <h3>ФОП у мережі пакета</h3>
          <p class="dr-hint">У цього пакета ${escapeHtml(uaNum(fop))} ${escapeHtml(plural(fop, "надавач", "надавачі", "надавачів"))}
            — фізичні особи-підприємці (${escapeHtml(pctUk(net ? fop / net * 100 : 0))} мережі), із сумою — ${escapeHtml(uaNum(fopPaid))}.
            У вивантажці в графі ЄДРПОУ в них стоїть літерал «ФОП», однаковий у всіх, тож усередині порталу вони
            розрізняються за нормалізованим ПІБ. Наслідок, який варто памʼятати: з <b>фактичними оплатами</b>
            (там ФОП стоять під РНОКПП) вони не зшиваються в принципі — містка між РНОКПП і ПІБ у публічних даних
            немає. У межах цієї вкладки ФОП пораховані нарівні з усіма.</p>
        </section>` : ""}

        <section class="dr-card dr-card-limits">
          <h3>Чого ця вкладка не може сказати взагалі</h3>
          <ul class="dr-limits">
            <li><b>Чи допомогло пацієнтам.</b> У джерелах немає результату лікування — ні наслідку, ні повторної
              госпіталізації, ні виживаності. Вкладка міряє контрактування і вихід, тобто <b>масштаб</b>, а не результат.</li>
            <li><b>Скільки людей охоплено.</b> Обсяги рахують послуги, а не унікальних пацієнтів: одна людина з
              пʼятьма візитами дає пʼять. Тому «інтенсивність», а не «% охоплення».</li>
            <li><b>Де живе пацієнт.</b> Область у всіх розрізах — місце <b>надавача</b>. У пакетах із кількома
              центрами на країну люди їдуть, і область центру вбирає чужі випадки.</li>
            <li><b>Чи діяв пакет увесь період.</b> Вивантажка не містить строків дії окремих пакетів у договорі,
              тож заклад, який пропрацював два місяці, стоїть поряд із тим, хто працював рік.</li>
            <li><b>Скільки грошей реально перераховано.</b> На вкладці — сума договорів. Фактичні оплати живуть в
              окремому наборі відкритих даних і в паспорт поки не заведені.</li>
          </ul>
        </section>`,
        };
      },
    ...
    anatomy: {
      ...
      build(pkg) {
        ...
        <section class="dr-lede da-lede">
          <div class="dr-lede-main">
            ...
            <p class="dr-lede-a">${escapeHtml([
              ...
              ANAT_TEAM[team.band],
              `перелік вимог ${ANAT_GATE[gate.band]}`,
              role ? ANAT_ROLE[role.band] : "",
            ].filter(Boolean).join(" · "))}</p>
            <p class="dr-lede-b">Осі на вкладці міряють <b>масштаб</b> — скільки роботи пакет робить по країні.
        ...
        ${axis("", "Обсяг вимог", ANAT_GATE, gate,
          "Скільки рядків вимог містить специфікація пакета",
          ...
          `Найдовший перелік вимог у ПМГ — у пакета ${escapeHtml(gTop)} (${uaNum(gateOf(all[gTop]))} рядків); цей пакет на ${gRank}-му місці з ${nums.length}.`,
          `<div class="da-nums">
        ...
        <section class="dr-card dr-card-limits">
          ...
          <ul class="dr-limits">
            <li>Це опис <b>вимог і складу</b>, а не якості. Довгий перелік вимог не означає, що пакет виконується добре.</li>
            ...
            <li>Обсяг вимог — це <b>кількість рядків</b> у тексті, а не їхня вартість і не оцінка складності входу. Один томограф в одному рядку важчий за десять рядків дрібного інвентарю.</li>
    ...
    core80: {
      ...
      build(pkg) {
        ...
        <section class="dr-lede" data-verdict="${v.key}">
          <div class="dr-lede-main">
            <p class="dr-lede-q">На скількох закладах тримається 4/5 суми договорів пакета</p>
            <p class="dr-lede-a"><b>${uaNum(s.core)}</b> ${zozUk(s.core)} із ${uaNum(s.n)} —
              <b>${pctUk(s.corePct)}</b> мережі — на них припадає <b>${pctUk(s.coreMoneyPct)}</b> суми договорів пакета.</p>
        ...
        <section class="dr-card dr-card-hero">
          <h3>Як розподілена сума договорів пакета</h3>
          <p class="dr-hint">Верхня смуга — заклади, нижня — законтрактовані гривні. Стрічка між ними показує, наскільки одне не збігається з іншим.</p>
          ${funnelSvg(s)}
          <div class="dr-legend">
        ...
        <section class="dr-stats">
          ${statCard("Якби ділилося порівну", `${uaNum(s.equalCount)} <small>ЗОЗ</small>`,
            "стільки закладів давали б 4/5 суми договорів при однакових договорах",
            "80 % суми договорів при рівному розподілі припадало б на 80 % закладів. Це природний нуль показника: чим менше за нього ядро, тим щільніше зібрані суми.")}
        ...
        <div class="dr-two">
          <section class="dr-card">
            ...
            <p class="dr-hint">Що вище крива над діагоналлю — то щільніше зібрані суми договорів.</p>
            ${curveSvg(s)}
          </section>
        ...
        <section class="dr-card">
          ...
          <p class="dr-hint">Скільки суми договорів пакета в кожному регіоні припадає на заклади ядра. Клік по регіону відфільтрує перелік ЗОЗ на сторінці.</p>
          ${regionsHtml(s)}
        </section>
  ...
  function applyCoreFilter(s) {
    passportState.hospitalCombo = {
      ...
      chip: "Фільтр:",
      label: `ядро бюджету — ${uaNum(s.core)} ${zozUk(s.core)}, на які припадає ${pctUk(s.coreMoneyPct)} суми договорів пакета`,
    };
