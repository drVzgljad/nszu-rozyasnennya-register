...
const INTENS_WHY =
  ...
  "Це окрема величина, а не складова масштабу: масштаб міряє обсяг роботи, а не її насиченість. " +
...
function renderPkgAxes(pkgNum, bench, myNet, myMoney, sharePMG) {
  ...
  let note = "";
  if (!myMoney) {
    note = "Осі сум договорів немає: у вивантажці договорів суми за цим пакетом відсутні.";
  } else if (!myVol) {
...
function renderAnalytics() {
  ...
  el("thermoKpis").innerHTML =
    kpiTileHtml("", "Покриття регіонів", `${oblCovered} <small>з ${oblTotal}</small>`,
      oblCovered === oblTotal ? "заклади в усіх регіонах" : `немає закладів у ${oblTotal - oblCovered} регіонах`) +
    kpiTileHtml("", "Медіанний договір", escapeHtml(formatMoneyShort(median)),
      noSums ? "у вивантажці суми за пакетом відсутні" : "типова сума на один заклад") +
    kpiTileHtml("", "Ядро: 80 % суми договорів", noSums ? "—" : `${core80} <small>ЗОЗ</small>`,
      noSums ? "у вивантажці суми за пакетом відсутні"
             : `на ${pctUk(core80Share)} мережі припадає 4/5 суми договорів пакета · топ-5 ЗОЗ — ${Math.round(top5Share)} %`,
      "", noSums ? "" : "core80") +
...
// Смуги осі — про ДОВЖИНУ переліку вимог, а не про складність входу: це
// кількість рядків у тексті специфікації, вона не міряє вартість обладнання,
// кадрову складність чи відповідність надавача (P1-03).
const ANAT_GATE = ["короткий", "середній", "довгий", "дуже довгий"];
...
const ANAT_BUYS = {
  ...
  "готовність": { icon: "", text: "оплата за готовність надавати допомогу, а не за випадок" },
...
/** Обсяг вимог — скільки рядків вимог містить специфікація: обладнання +
    організаційні умови + кадри. Смуга — за місцем серед 46 пакетів, бо
    абсолютне число нічого не каже. Це НЕ оцінка складності чи вартості входу. */
...
function renderAnatomy() {
  ...
  el("anatAxes").innerHTML =
    ...
    anatAxisHtml("🧗", "Обсяг вимог", ANAT_GATE, gate, gate.tip) +
  ...
  el("anatFootnote").textContent =
    "Осі навмисно не зводяться в один бал: вони незалежні одна від одної й від масштабу. " +
    "Вузький пакет буває найдовшим за переліком вимог (неонатальний скринінг — один напрям і 132 рядки вимог), " +
    "а великий за мережею — найкоротшим. Клінічна ширина й обсяг вимог беруться з тексту специфікації " +
...
function exportPassportToExcel() {
  ...
  const overviewData = [
    ...
    [],
    ["Секція", "Сума договорів (₴)", "Кількість закладів"],
  ];
