...
(() => {
  ...
  function renderDx(raw) {
    ...
    if (o.ids.length) {
      ...
      const isManif = d.manif.length > 0;
      html += `<dt>${isManif
        ? 'Групи, до яких випадок потрапить, якщо цей код <em>поставити</em> основним'
        : 'Досяжні ДСГ пакета 3/47 через ці ОДК'}
        <span class="mut small">(${gr.length})</span></dt><dd>${
        gr.map((g) => `<span class="chip" data-gr="${g.c}" title="${esc(g.name)}">${g.c}
          <span class="w">${esc(g.w || '')}</span></span>`).join(' ') || '—'}${
        isManif ? `<p class="small mut" style="margin:6px 0 0">Код присутній у переліку
          ОДК Таблиці співставлення, тому групер його приймає і помилки не показує.
          Це не робить його допустимим основним діагнозом за МКХ-10.</p>` : ''}</dd></dl>`;
    } else {
  ...
  function renderPair(grRaw, dxRaw) {
    ...
    if (shared.length && d.manif.length) {
      /* Розбіжність двох рівнів: за МКХ-10 код прояву основним бути не може, але
       * в переліку ОДК Таблиці співставлення він присутній, тож групер його
       * приймає. Саме тому система «не видає помилку» — але це не підтвердження
       * правильності кодування. Зеленим такий випадок позначати не можна. */
      html += `<div class="verdict v-warn"><b>${esc(gcode)} × ${esc(dcode)}:
        група досяжна, але пара некоректна.</b>
        Код ${esc(dcode)} присутній у переліку ${shared.map((id) => esc(id)).join(', ')}
        Таблиці співставлення, тому групер веде випадок до ${esc(gcode)}
        (ваговий коефіцієнт <b>${esc(g.w || '—')}</b>) і помилки не показує.
        Проте основним діагнозом цей код не зазначається, тож оплата за такою
        парою може бути переглянута за результатами моніторингу.</div>`;
