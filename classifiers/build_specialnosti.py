...
def main():
    ...
    # Вада джерела лишається на видноті, хоча вимога вже зіставляється: назву
    # для зіставлення відновив pkg_staff (загублена скісна риска), а сам текст
    # специфікації як був хибний, так і лишився — правити акт ми не можемо.
    typo = [f"пакет {r['package']}: {r['raw'][:160]}" for r in reqs if r.get("repaired")]
    if typo:
        defects.append({
            "source": "специфікації пакетів ПМГ-2026",
            "issue": "у назві посади загублено скісну риску — «Сестра "
                     "медичнабрат медичний» замість «Сестра медична/брат "
                     "медичний». Для зіставлення назву відновлено (склейка "
                     "однозначна), але у тексті специфікації помилка лишається",
            "items": typo,
        })
