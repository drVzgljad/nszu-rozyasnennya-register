...
ПАСТКИ (набиті в build_posady.py, перенесені сюди разом із кодом):
  ...
  6. У двох вимогах джерело загубило скісну риску рівно там, де вона мала
     стояти: «Сестра медичнабрат медичний» замість «Сестра медична/брат
     медичний» (пакети 8 і 63). Без відновлення ці вимоги не зіставляються з
     Переліком узагалі, і на екрані це читається як «такої посади не існує» —
     хоча вона є. Склейка однозначна («медична» + «брат»), тож назву для
     ЗІСТАВЛЕННЯ відновлюємо, а дослівний текст вимоги (raw) лишаємо як в акті:
     виправляти акт ми не маємо права, лише читати його правильно. Факт
     відновлення позначено прапорцем repaired, щоб вада джерела не зникла
     з-перед очей.
"""
import json
import re
...
# Загублена скісна риска в парній назві (пастка 6). Склейка можлива тільки в
# одному місці — між «медична/медичний» і «брат/сестра», — тож відновлення
# однозначне і нічого не вгадує.
GLUE = re.compile(r"(медичн(?:а|ий))(брат|сестра)", re.I)


def unglue(name):
    """Повертає назву зі скісною рискою, якщо джерело її загубило."""
    return GLUE.sub(lambda m: m.group(1) + "/" + m.group(2), name)
...
def load_requirements(path):
    ...
    Кожен запис:
      ...
      repaired — у назві відновлено загублену скісну риску (пастка 6)
    """
    if not path.exists():
        return []
            ...
            for sec in unit.get("sections", []):
                ...
                for it in sec.get("items", []):
                    ...
                    out.append({
                        ...
                        "head": name,
                        "cond": cond,
                        # Зіставляємо за відновленою назвою, показуємо — за
                        # дослівною: raw і head лишаються такими, як в акті.
                        "alts": split_names(fixed),
                        "alts_full": split_names(fixed, keep_tail=True),
                        "repaired": fixed != name,
